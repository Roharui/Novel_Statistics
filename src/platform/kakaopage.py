...
class Kakaopage(Platform):
  ...
  def __searchTitleBody(self, text: str):
    return {"query": TITLE_QUERY, "operationName":"SearchKeyword","variables":{ "input": { "size": 100, "keyword": text, "categoryUid": "11"}}}

  def __searchLinkBody(self, serial: int):
    return {"query": LINK_QUERY, "operationName":"contentHomeOverview","variables":{"seriesId":serial}}

  def __searchBookBody(self, serial: int):
    return {"query": BOOK_QUERY, "operationName":"contentHomeProductList","variables":{"seriesId":serial,"boughtOnly":False}}
  ...
  async def __parseResult(self, novel_content) -> Result:
    ...
    is_plus = True
    age_limit = 0 if content["ageGrade"] == "All" else 15 if content["ageGrade"] == "Fifteen" else 19
  ...
  async def __searchCount(self, serial: int):
    content = json.loads(await self._postContent(self.SEARCHLINK, data=json.dumps(self.__searchBookBody(serial))))
    return content["data"]["contentHomeProductList"]["totalCount"]

  # 소설 제목으로 검색
  async def searchTitle(self, title: str) -> List[Result]:
    content = json.loads(await self._postContent(self.SEARCHLINK, data=json.dumps(self.__searchTitleBody(title))))
  ...
  async def searchURL(self, url: str) -> Result:
    ...
    content = json.loads(await self._postContent(self.SEARCHLINK, data=json.dumps(self.__searchLinkBody(serial))))
