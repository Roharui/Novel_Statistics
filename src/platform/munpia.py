...
class Munpia(Platform):
    ...
    async def searchEpisode(self, url: str) -> List[Episode]:
        ...
        while True:
            try:
                ...
                pLst = (
                    content.find("div", {"class": "pagination"})
                    .find("ul")
                    .find_all("li")
                )
    ...
    async def searchURL(self, url: str) -> Result:
        ...
        try:
            ...
            thumbnail = (
                "https:"
                + novel_content.find("div", {"class": "dt cover-box"})
                .find("img")["src"]
                .strip()
            )
            ...
            author = (
                ...
                .find("dd")
                .find("a")
                or novel_content.find("dl", {"class": "meta-author meta"}).find("dd")
            ).text.strip()
            ...
            tags = (
                []
                if tag_wrap == None
                else [Tag(name=tag.text.strip()) for tag in tag_wrap.find_all("a")]
            )
            ...
            number_text = novel_content.find_all("dl", {"class": "meta-etc meta"})[
                1
            ].text
            number_list_1 = [
                "".join(i for i in x if i.isdigit()) for x in number_text.split("\n")
            ]
